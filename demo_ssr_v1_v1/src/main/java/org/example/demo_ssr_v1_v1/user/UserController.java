...
//    }
    ...
    // 로그인 인터셉터에서 막고 있음 -> WebMvcConfig 에서 수정
    // [흐름] 1. 인가 코드 받기 -> 2. 토큰 발급 요청 (JWT) -> 3. JWT 으로 사용자 정보 요청 -> 서버에 로그인/회원가입 처리
    ...
    public String kakaoCallback(@RequestParam(name = "code") String code, HttpSession session) {
        ...
        tokenParams.add("client_id", "4fb7df49f139380216547fbe97296f4a");
        ...
        // 2-6. 요청하고 JWT 토큰 응답 받기 (카카오에서 엑세스 토큰이라 부름)
        ...
        // 새로 생성한 username이 DB에 있다면 -> 회원가입 함
        // 사용자 이름 조회 쿼리 수행
        ...
        if(userOrigin == null) {
            ...
            // 조심 - 반드시 필요함
            userOrigin = newUser; // 반드시 넣어 줘야 함 -> 로그인 처리 해야 함
        } else {
            System.out.println("이미 가입된 회원 입니다. 바로 로그인 처리 진행합니다.");
        }
    ...
    public String deleteProfileImage(HttpSession session) {
        ...
        // 왜 user를 다시 받을까? -- 세션 정보가 (즉 프로필이 삭제되었기 때문에)
