...
public class User {
     ...
     *
     * CascadeType.ALL
     * - 운명 공동체 User를 저장하면 Role 로 자동 저장되고,
     * User를 삭제하면 가지고 있던 Role 들도 다 같이 삭제됨
     * (홍길동 (관리자, 일반사용자) 삭제하면 userRole 2가지 row도 자동 삭제 됩니다.
     *
     * orphanRemoval = true
     * 리스와 DB의 동기화 입니다.
     * java의 roles 리스에서 요소(Role)를 .remove() 하거나 .clear() 하면
     * DB 에서도 해당 데이터(DELETE)가 실제로 처리 됩니다.
     */
    // 나중에 다른 개발자가 findById(쿼리 메서드 호출할 때 신경 쓸 필요없이 전부 role 까지 변환 해줌)
    ...
    // getProfilePath 변수를 호출하면 알아서 세팅 되게 하고 싶다
