...
import org.example.demo_ssr_v1_v1._core.errors.exception.Exception400;
import org.example.demo_ssr_v1_v1._core.errors.exception.Exception403;
import org.example.demo_ssr_v1_v1._core.errors.exception.Exception404;
import org.example.demo_ssr_v1_v1._core.errors.exception.Exception500;
import org.example.demo_ssr_v1_v1._core.utils.FileUtil;
...
//            UserResponse.LoginDto dto = new UserResponse.LoginDto()
    ...
    public User 회원정보수정(UserRequest.UpdateDTO updateDTO, Long userId) {
        ...
        // 새로운 이미지 파일을 생성하고 기존에 있던 이미지 파일ㅇ르 삭제해야 한다
        ...
        // 객체 상태값 변경 (트랜잭션이 끝나면 자동으로 commit 및 반영해 줌)
    ...
    public User 프로필이미지삭제(Long sessionUserId) {
        ...
        // 3. 프로필 이미지가 있다면 삭제 (FileUtil) 헬퍼 클래스 사용 할 예정 (디시크에서 삭제)
